...
| 5 | Pause 60 s → Play | rejoins LIVE edge (compare against web player) | **PASS** | 2026-07-02 / DHU + adb, `efbdf2a` | Paused 65 s via `adb shell media dispatch pause`, played: audio matched the live web player (the station's web player side-by-side), not the stale buffer — the >30 s staleness reload fired as designed. |
...
5. **Live edge:** play ≥ 1 min → pause 60 s → play; compare audio against the station's own web player — it must rejoin the live edge, not resume the stale buffer.
